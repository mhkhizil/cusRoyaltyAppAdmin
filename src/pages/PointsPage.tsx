...
export function PointsPage() {
  ...
  const [calculationType, setCalculationType] = useState<"FLAT" | "SPEND_BASED">(
    "FLAT"
  );
  ...
  const handleCreateRule = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await createRule({
        ...
        calculationType,
        flatPoints:
          calculationType === "FLAT" && flatPoints !== ""
            ? Number(flatPoints)
            : undefined,
        spendUnit:
          calculationType !== "FLAT" && spendUnit !== ""
            ? Number(spendUnit)
            : undefined,
        pointsPerSpendUnit:
          calculationType !== "FLAT" && pointsPerSpendUnit !== ""
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="mt-4 space-y-3">
            ...
            >
              <option value="">{t("points.scan.fields.locationPlaceholder")}</option>
              {branches.map((branch) => (
            ...
            <QrScanner
              ...
              paused={isProcessingScan || isLoading}
            />
          ...
          {lastScanResult ? (
            <div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900 dark:border-emerald-900/40 dark:bg-emerald-950/30 dark:text-emerald-200">
              <p className="font-semibold">{t("points.scan.resultTitle")}</p>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5 dark:border-slate-800 dark:bg-slate-900">
          ...
          <form className="mt-4 grid gap-3 sm:grid-cols-2" onSubmit={handleCreateRule}>
            ...
            <select
              ...
              onChange={(event) =>
                setCalculationType(
                  event.target.value === "FLAT" ? "FLAT" : "SPEND_BASED"
                )
              }
            >
              <option value="FLAT">{t("points.rules.calculation.flat")}</option>
              <option value="SPEND_BASED">
                {t("points.rules.calculation.spendBased")}
              </option>
            </select>
      ...
      {isLoading && rules.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="border-b border-slate-200 px-4 py-3 text-sm text-slate-500 dark:border-slate-800 dark:text-slate-400">
            {t("points.rules.total", { count: rules.length })}
          </div>
          <div className="overflow-x-auto">
            <table className="min-w-[36rem] w-full text-left text-sm md:min-w-full">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {rules.map((rule) => (
                  <tr key={rule.id} className="text-slate-700 dark:text-slate-200">
                    <td className="px-4 py-3 font-medium">{rule.name || "—"}</td>
                    <td className="px-4 py-3">{rule.calculationType || "—"}</td>
                    <td className="px-4 py-3">
                      {rule.isFlat()
                        ? rule.flatPoints ?? "—"
                        : `${rule.pointsPerSpendUnit ?? "—"} / ${rule.spendUnit ?? "—"}`}
                    </td>
                    <td className="px-4 py-3">{rule.minimumPurchase ?? "—"}</td>
                    <td className="px-4 py-3">
